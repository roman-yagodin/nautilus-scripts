...
if (!Env.IsEmpty("R7_SCRIPTING_FORK"))
{
   ...
   try 
   {
      ...
      // Read publican.cfg
      var CONFIG = File.ReadAllText(Path.Combine(BOOK_PATH, "publican.cfg"));
      ...
      Command.Run (NauHelper.ScriptDirectory + "/publican/f2svg.cs", "", int.MaxValue);
	      ...
	      File.Copy(chapter, "ru-RU/.chapters/" + Path.GetFileName(chapter) + "t");
            ...
            //   chapter, HOME + "/.gnome2/nautilus-scripts/publican/set_condition.xslt"));
      	   ...
      	   Command.Run("xsltproc", string.Format("--output \"{0}t\" --xinclude --novalid \"{1}\" \"{0}\"", 
      	    chapter, HOME + "/.gnome2/nemo-scripts/publican/xslt/slides.xslt"));
    ...
    */
      ...
      do
      {
         exitCode = Command.Run ("publican", string.Format("build --formats {0} --langs ru-RU", XFORMATS));
